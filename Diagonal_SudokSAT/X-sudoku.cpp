...
// 挖洞法生成数独初盘
void DigHole(Answer *ans, int que[][10], VarTable var_watch[])
{
    int hole[90];
    // 随机数生成器
    mt19937 g(rand());
...
void CreateSudoku(int sud[][10], int que[][10])
{
    ...
    do
    {
        ...
        for (int i = 1; i < 10; i++)
        {
            // 随机填入一个数字
            int value = rand() % 9 + 1;
            while (isvalid(sud, 1, i, value) == FALSE)
                value = value % 9 + 1;
            int var = (i - 1) * 9 + value;
            sud[1][i] = value;
            ans->value[var] = 1;
    ...
    } while (DPLL(ans, var_watch, 1, 1) != SATISFIABLE);
    int v1 = rand() % 9 + 1, v2 = rand() % 9 + 1;
    // 将ans转换为数独
    for (int i = 1; i <= 729; i++)
    {
        if (ans->value[i] > 0)
