...
void CreateClause(Clause *clause, VarTable war_watch[], Answer *ans, string s, int &varclausenum)
{
    ...
    clause->cnt = cnt;
    VarList *p = clause->headvar->nextvar;
    while (p != NULL)
    {
        int da = p->data;
        if (da > 0)
            firstbranch[da] += float(var_num) / (cnt * cnt);
        else
            firstbranch[var_num - da] += float(var_num) / (cnt * cnt);
        p = p->nextvar;
    }
}
